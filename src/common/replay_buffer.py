...
class ReplayBuffer:
    def __init__(self, args):
        ...
        else: # in hierarchical architecture
            self.buffer = {
                'o': np.zeros(
                    [self.args.buffer_size, self.args.episode_limit, self.args.n_ally_agent_in_platoon, self.args.obs_shape]),
                's': np.zeros(
                    [self.args.buffer_size, self.args.episode_limit, self.args.state_shape]),
                'a': np.zeros(
                    [self.args.buffer_size, self.args.episode_limit, self.args.n_ally_agent_in_platoon, 1]),
                'onehot_a': np.zeros(
                    [self.args.buffer_size, self.args.episode_limit, self.args.n_ally_agent_in_platoon, self.args.n_actions]),
                'avail_a': np.zeros(
                    [self.args.buffer_size, self.args.episode_limit, self.args.n_ally_agent_in_platoon, self.args.n_actions]),
                'r': np.zeros(
                    [self.args.buffer_size, self.args.episode_limit, 1]),
