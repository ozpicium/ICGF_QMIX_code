...
class StarCraft2Env_HC(MultiAgentEnv):
    ...
    def __init__(
        ...
        self.final_SP_to_reach = final_SP_to_reach
        
        self.tank_maps = []
        for k in self.allmaps.keys():
          self.tank_maps += self.allmaps[k]
        ...
        if self.map_name in self.allmaps['multiscenario_maps'] + self.allmaps['_13sp_3path_maps']:
          self.only_local_enemy_obs = 1 #reduce input and action vector lenghts of each agent to only include the features corresponding
                                          #to locally visible enemies
          self.only_local_enemy_state = 1 #reduce global state to only include features corresponding to locally visible enemies
    ...
    def reset(self):
        ...
        return self.get_obs_company(), self.scenario_id
    ...
    def init_platoons(self):
        ...
        while True:
            ...
            if len(self.ally_platoons[0]) < self.n_ally_agent_in_platoon:
              print('GLITCH-------- platoon is incomplete!')
              self.glitch = True
              return
            ...
            if self.map_name in self.allmaps['multiscenario_maps']:
              if enemy_units == []:
                self.scenario_id = 0
              else:
                enemy_units_sorted = sorted(
                  enemy_units,
                  key=attrgetter("unit_type", "pos.y"),  # sort the enemies by y position
                  reverse=False,
                )
                enemy_SP = self.get_platoon_current_sp(enemy_units_sorted[-4:])
                if enemy_SP == 1:
                  self.scenario_id = 1
                elif enemy_SP == 4:
                  self.scenario_id = 2
            ...
            if len(self.enemies) != 0:  # the map has some enemies
                ...
                if self._episode_count == 0:
                    if self.map_name in self.allmaps['_8sp_no_enemy']:  # these scenarios doesn't have enemies
            ...
            if self.map_name in self.allmaps['multiscenario_maps'] + self.allmaps['_13sp_3path_maps'] + self.allmaps['_8sp_1path_maps']:
    ...
    def update_platoons(self):
        ...
        if self.hierarchical:
            ...
            check_arrived_at_target = [i[self.final_SP_to_reach-1] for i in self.platoons_move_record]
            if check_arrived_at_target.count(1) == self.n_ally_platoons:
                arrived_at_target = True
              

            if self.map_name in self.allmaps['multiscenario_maps'] + self.allmaps['_13sp_3path_maps'] + self.allmaps['_8sp_1path_maps']:
                if self._episode_steps == self.episode_limit and not arrived_at_target:
                    return -1, arrive_reward #lost
                    
                elif (sum(n_ally_alive) == 0 and n_enemy_alive > 0):
                    return -1, arrive_reward #lost
                
                elif (sum(n_ally_alive) > 0 and (arrived_at_target)  # and n_enemy_alive == 0
                      ):
            ...
            elif self.map_name in self.allmaps['_8sp_no_enemy']:
                if self._episode_steps == self.episode_limit and not arrived_at_target:
                    return -1, arrive_reward #lost
    ...
    def get_target_SP_id(self, init_platoon_SPs):
        ...
        if self.map_name in self.allmaps['multiscenario_maps'] + self.allmaps['_13sp_3path_maps']: ## the following code is added by Shubham to ensure that each path is selected with equal probability
